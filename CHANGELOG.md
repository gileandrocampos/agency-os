...
### Added
- `Queue`: fila persistente em SQLite (`better-sqlite3`) com suporte a enqueue, list, status, retry automático e processamento sequencial via `queue:add`, `queue:run`, `queue:list` e `queue:status`
